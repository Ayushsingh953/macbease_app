...
import { border, center, row, row_even } from "../../../constants/styles";
...
function HistoryModal({ setHistoryModal }) {
  ...
  useEffect(() => {
    async function getHistory() {
      try {
        const res = await axios.get(`${URL}/props/getPropOrder`, config);
        console.log(res.data.propOrder);
        setHistoryData(res.data.propOrder);
      } catch (err) {
        console.log(err);
      }
    }
...
const getGreenDot = (item) => {
  if (item.status === "Booked" || item.status === "Yet to be dispatched") {
    return <View style={[styles.greenDot, { marginRight: 12 }]}></View>;
  }
};
