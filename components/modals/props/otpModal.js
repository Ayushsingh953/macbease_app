...
import { border, center } from "../../../constants/styles";
...
function OtpModal({ color, setModalActive, setModalState }) {
  ...
  const bookedId = propCtx.bookedId;
  const otp = propCtx.otp;
  ...
  return (
    <View style={[styles.modalContainer, { borderColor: color }]}>
      ...
      >
        <Text style={[styles.text]}>OTP is {otp}</Text>
        <Text style={[styles.text]}>Prop Id is {bookedId}</Text>
      </View>
      <View style={[styles.animationContainer]}>
        <LottieView
          source={require("../../../assets/greenTick.json")}
          autoPlay
        />
      </View>
      <View style={[styles.bottomText, center]}>
        <Text style={[styles.text, { color: "#8a8a86" }]}>
          See you on the dispatch center...
        </Text>
      </View>
    </View>
  );
